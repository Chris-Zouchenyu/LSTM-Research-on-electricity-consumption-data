...
def cnn_model(train, sw_width, epochs_num, batch_size, in_start=0, verbose_set=0):
    '''
    该函数调用1D CNN模型
    '''
    mode = 'train'
...
# cnn_model(train = train, sw_width = 7, epochs_num=20)
...
def forecast(model, pred_seq, sw_width):
    ...
    input_x = input_x.reshape((1, 1, -1)) # 重塑形状[1, 1, -1]
    # 重塑
...
# train,test = split_dataset(dataset)
# model = cnn_model(train = train, sw_width = 7,epochs_num=500)
...
def main_run(dataset, sw_width, days, name, in_start, epochs_num, batch_size, verbose):
    '''
    主函数：数据处理、模型训练流程
    '''
    # 划分训练集和测试集
    ...
    # 训练模型
    model = cnn_model(train, sw_width, epochs_num, batch_size, verbose_set=0)
    # 计算RMSE
...
if __name__ == '__main__':
    ...
    main_run(dataset, sw_width = sliding_window_width, days = days, 
             name = name, in_start = 0, epochs_num = epochs_num, batch_size = batch_size, verbose = 0)
